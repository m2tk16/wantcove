...
export function ProductEditor({ product, busy, onCancel, onSave }: {
  ...
  return <form className="product-editor" onSubmit={submit}>
    ...
    <div className="form-grid">
      ...
      <label className="wide-field">Image location<input maxLength={2048} onChange={(event) => update('imageUrl', event.target.value)} pattern="/products/[a-z0-9]+(?:[.-][a-z0-9]+)*\.(?:avif|jpg|jpeg|png|webp)" placeholder="/products/item.webp" required title="Use a first-party /products/ AVIF, JPEG, PNG, or WebP path." value={draft.imageUrl} /></label>
    ...
    <p className="form-note">New products are always saved as drafts. Product images must use a first-party /products/ path. Retailer URLs stay inactive on the public site until the affiliate launch gate is approved.</p>
