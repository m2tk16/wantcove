...
| P1 Security | First-party product image storage and CSP | High / Medium | Local candidate | Amplify Hosting headers, a deny-by-default CSP, backend-enforced first-party `/products/` paths, and an explicit legacy-path compatibility map are implemented locally. Protected Beta deployment and hosted header/network acceptance remain. |
...
| P3 High/Low | Optimize generated catalog images | High / Low | Local candidate | Replaced four large PNGs with 24 responsive AVIF/WebP variants totaling under 1 MB; intrinsic sizing, `srcset`, lazy loading, and automated budgets are implemented locally. Beta visual and transfer acceptance remain. |
