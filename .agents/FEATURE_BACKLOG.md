...
| P2 Bugs | Authenticated-admin Identity Pool like fallback | Medium / Medium | Fix in progress | A failed cloud read could recreate the loader and trigger repeated GraphQL requests from visible Like buttons. The local candidate stabilizes the loader and proves duplicate cards make one failed request; the underlying signed-in Identity Pool fallback still requires hosted diagnosis. |
| P1 Security | Affiliate disclosure and safe outbound-link policy | High / Low | In progress | Associate ID `wantcove-20` and pending-review status are recorded. The exact site identification statement is added locally; before live links, use Amazon-provided Special Links, retain near-link disclosure, and remove manually asserted ratings/prices unless supplied under current Program Content rules. |
...
| P3 High/Low | Move catalog reads to GraphQL | High / Low | In progress | Four-record hosted parity passed. The local phase-two candidate removes fixture merging, fails closed on live-read errors, and makes GraphQL the production catalog source; protected Beta deployment remains. |
| P3 High/Low | Migrate starter fixtures into Product storage | High / Low | Beta accepted | The idempotent administrator migration created exactly four published records; public GraphQL and hosted UI parity passed with no affiliate fields. Fixture compatibility removal is the separate local release candidate. |
| P3 High/Low | Manual Amazon SiteStripe product intake | High / Low | Deployed to Beta | Admin intake is live behind `ADMINS`; Associate ID `wantcove-20` is recorded. Public retailer actions stay inert pending verified Special Links, disclosure, content, and legal launch gates. |
