...
| P1 Security | Self-service administrator password recovery | High / Low | Local candidate | Added a dedicated Cognito email-recovery route with account-neutral request responses, client-side password confirmation, no secret logging/storage, and unchanged MFA plus `ADMINS` authorization. Full authentication gate and hosted Beta acceptance remain required. |
