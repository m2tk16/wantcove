...
| P1 Security | Self-service administrator password recovery | High / Low | Proposed | Add Cognito forgot-password and confirmation-code routes with generic responses that do not reveal whether an account exists. Preserve MFA, rate limits, auditability, and the server-enforced `ADMINS` boundary; never handle recovery secrets in logs or support messages. |
