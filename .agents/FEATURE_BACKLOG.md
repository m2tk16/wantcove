...
| P2 Bugs | Authenticated-admin Identity Pool like fallback | Medium / Medium | Corrective candidate | Root cause remains the `ADMINS` preferred role missing generated like grants. Beta job 20 exposed and safely stopped a nested-stack cycle before deployment; the correction creates the two-field IAM policy in Data, which already owns AppSync and depends on Auth. Hosted Beta acceptance remains required. |
