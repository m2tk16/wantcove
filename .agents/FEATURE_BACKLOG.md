...
| P1 Security | Cognito authentication UI and session handling | High / Medium | Implemented locally | Admin email/password, temporary-password continuation, TOTP setup/challenge handling, required MFA, and disabled self-registration are in the release candidate; cloud user creation and Beta acceptance remain. |
| P1 Security | Admin-only catalog access and server-enforced product mutations | High / Medium | Implemented locally | `/admin` supports create/update/publish/archive/guarded delete; Product model writes are denied and a bounded Function rechecks `ADMINS`, validates input, and uses conditional writes. |
...
| P3 High/Low | Manual Amazon SiteStripe product intake | High / Low | Implemented locally | Admin intake accepts ASIN, Amazon URL, HTTPS image, alt text, curated copy, category, rank, and lifecycle state. Public retailer actions remain inert pending enrollment and legal launch gates. |
