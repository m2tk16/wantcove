...
| P3 High/Low | Migrate starter fixtures into Product storage | High / Low | In progress | Phase one adds an idempotent, non-overwriting admin migration for four published records with no affiliate destinations. Deploy to Beta, invoke and verify parity, then remove fixture fallback and the legacy-like allowlist in a separate release. |
