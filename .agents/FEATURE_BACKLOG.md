...
| P2 Bugs | Starter migration resolver dispatch | High / Low | Fix in progress | Beta job 16 deployed successfully, but the first migration attempt proved Amplify sends the operation name as top-level `fieldName`. Correct the shared Function dispatch, preserve server-controlled routing, redeploy through the protected flow, and repeat hosted acceptance. |
| P2 Bugs | Authenticated-admin Identity Pool like fallback | Medium / Medium | Investigate | Anonymous consented likes persist correctly, but likes fall back to session-only while the private Cognito administrator session is active. Resolve before public accounts or routine signed-in browsing. |
...
| P3 High/Low | Move catalog reads to GraphQL | High / Low | In progress | Phase-one candidate adds guarded starter migration while published managed products continue to merge over fixtures. Complete only after hosted parity and a separate compatibility-removal release. |
| P3 High/Low | Migrate starter fixtures into Product storage | High / Low | In progress | Phase one is deployed to Beta, but the first invocation exposed the tracked resolver-dispatch bug before any data write. Deploy the correction, invoke and verify four-record parity, then remove fixture fallback and the legacy-like allowlist separately. |
