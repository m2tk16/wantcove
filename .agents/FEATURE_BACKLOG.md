...
| P2 Bugs | Authenticated-admin Identity Pool like fallback | Medium / Medium | Fix in progress | Root cause confirmed: Cognito selects the `ADMINS` group preferred IAM role, while generated like-field grants existed only on the generic authenticated role. The candidate grants that group role only `getViewerProductLike` and `setViewerProductLike`; Beta hosted acceptance remains required. |
